...
if __name__ == '__main__':
    ...
    for line in Lines:
        ...
        if value[0] not in truth:
            truth[value[0]] = []
        tokens_e1, tokens_e2, ancestor_token = check_interaction(all_parse[value[0]], all_entities[value[0]], value[1],
                                                                 value[2])
        aux = (("Sentence: " + sentences[value[0]],
                ...
                "Ancestor:",
                # "Ancestor: " + str(ancestor_token)), value[-1])
                ancestor_token), value[-1])
    ...
    missing_dict = {}
    count_missing_ancestors = {}
    for ent, type in missing:
        if type[:-1] not in missing_dict:
            missing_dict[type[:-1]] = []
            count_missing_ancestors[type[:-1]] = {}
        missing_dict[type[:-1]].append(ent)
        count_missing_ancestors[type[:-1]][ent[-1]['lemma']] = count_missing_ancestors[type[:-1]].get(ent[-1]['lemma'],
                                                                                                      0) + 1

    for key, value in count_missing_ancestors.items():
        count_missing_ancestors[key] = {k: v for k, v in
                                        sorted(count_missing_ancestors[key].items(), key=lambda item: item[1],
                                               reverse=True)}

    print("Missing ancestors:", count_missing_ancestors)
    ...
    analyze_pairs.plot_ancestor_count(count_missing_ancestors)
